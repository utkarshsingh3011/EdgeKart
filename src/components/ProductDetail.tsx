...
export const ProductDetail: React.FC<ProductDetailProps> = ({
  ...
  // Fetch product data dynamically using productService.getProductById(id) -> GET /api/products/:id
  useEffect(() => {
    let isMounted = true;
    setIsLoading(true);
    setIsNotFound(false);
    ...
    const fetchProductFromApi = async () => {
      try {
        ...
        if (isMounted && response && response.success && response.data) {
          ...
          // Fetch related components by category from backend API
          try {
            if (raw.category) {
        ...
        console.error(`Error fetching product details from API for id "${id}":`, error);
        if (isMounted) setIsNotFound(true);
      } finally {
  ...
  // Render skeleton during API fetch loading state
  if (isLoading) {
    return <SkeletonProductDetail theme={theme} />;
  }

  // Render friendly 404 error state if product is not found or API errors
  if (isNotFound || !product) {
    return (
      <motion.div
        initial={{ opacity: 0, y: 15 }}
        animate={{ opacity: 1, y: 0 }}
        className={`min-h-[75vh] py-24 flex items-center justify-center transition-colors duration-300 ${
          theme === 'dark' ? 'bg-slate-950 text-slate-100' : 'bg-slate-50 text-slate-900'
        }`}
      >
        <div className="max-w-md w-full text-center px-4">
          <div className={`mx-auto w-20 h-20 rounded-3xl border flex items-center justify-center mb-6 shadow-xl ${
            theme === 'dark' ? 'bg-slate-900/80 border-slate-800 text-rose-400' : 'bg-white border-slate-200 text-rose-500'
          }`}>
  ...
  const isFavorited = isWishlisted(product.id || (product as any)._id);
  ...
  const handleRelatedProductClick = (productIdVal: string) => {
    navigate(`/products/${productIdVal}`);
  };
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <button
          onClick={() => navigate('/')}
          className={`mb-6 inline-flex items-center space-x-2 text-xs font-semibold py-2 px-3.5 rounded-xl border transition-all cursor-pointer ${
            theme === 'dark'
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start mb-16">
          ...
          <div className="lg:col-span-6 flex flex-col space-y-6">
            ...
            <div
              ...
              className={`relative aspect-square rounded-3xl border overflow-hidden flex items-center justify-center p-8 cursor-zoom-in ${
                theme === 'dark'
              ...
              {imageVariants.map((imgType) => {
                ...
                return (
                  <button
                    ...
                    className={`p-3 rounded-xl border flex flex-col items-center justify-center space-y-2 transition-all cursor-pointer relative overflow-hidden ${
          ...
          <div className="lg:col-span-6 space-y-6 text-left">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className={`flex items-center space-x-1.5 text-xs font-bold ${
                isInStock ? 'text-emerald-500' : 'text-rose-500'
              }`}>
            ...
            <div>
              ...
              {product.sku && (
                <p className="text-xs text-slate-500 mt-2 font-mono uppercase tracking-wider">SKU: {product.sku}</p>
              )}
            </div>
            ...
            {/* Price */}
            <div>
              <span className="text-xs text-slate-400 font-semibold uppercase tracking-wider block">Special Price</span>
              <div className="flex items-baseline space-x-3">
                <span className="text-4xl font-black text-blue-500 font-sans">₹{product.price}</span>
                {originalPrice > product.price && (
                  <>
                    <span className="text-xs text-slate-400 font-semibold line-through">₹{originalPrice}</span>
            ...
            {/* Action Bar: Quantity, Wishlist, Add to Cart, Buy Now */}
            <div className={`p-5 rounded-2xl border space-y-4 ${
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-3.5">
                  ...
                  <div className={`flex items-center rounded-xl border p-1 ${
                    theme === 'dark' ? 'border-slate-800 bg-slate-950/60' : 'border-slate-250 bg-white'
                  }`}>
                    <button
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 pt-2">
                <button
                  ...
                  disabled={!isInStock}
                  className="w-full bg-slate-900/60 hover:bg-slate-800/80 text-blue-400 border border-slate-800 font-bold py-3.5 px-6 rounded-xl transition-all duration-300 transform active:translate-y-0 flex items-center justify-center space-x-2.5 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                >
                ...
                <button
                  ...
                  disabled={!isInStock}
                  className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-3.5 px-6 rounded-xl transition-all duration-300 transform hover:-translate-y-0.5 shadow-lg shadow-blue-500/20 active:translate-y-0 flex items-center justify-center space-x-2.5 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 border-t border-slate-800/10 pt-16 mb-20 text-left">
          ...
          {/* Right Sub-Column: Technical Specifications Table */}
                ...
                <table className="w-full text-left border-collapse text-sm">
                  <tbody>
                    {Object.entries(product.specifications ?? product.specs ?? {}).map(([key, val], idx) => (
                      ...
                      >
                        <td className="py-3 px-4 font-bold text-slate-450 w-2/5">{key}</td>
              ...
              {relatedProducts.map((p) => {
                ...
                return (
                  <motion.div
                    ...
                    onClick={() => handleRelatedProductClick(relId)}
                    className={`p-5 rounded-2xl border text-left flex flex-col justify-between group transition-all duration-300 cursor-pointer relative ${
                      theme === 'dark'
                  ...
                  >
                    <div>
                      <span className={`absolute top-4 left-4 z-10 text-[9px] font-extrabold uppercase tracking-wider px-2.5 py-1 rounded-full ${
                        theme === 'dark' ? 'bg-slate-950/90 text-blue-400 border border-slate-800' : 'bg-blue-50/90 text-blue-600 border border-blue-100'
                      }`}>
                        {p.category}
                      </span>

                      <div className={`relative aspect-square p-6 flex items-center justify-center overflow-hidden border-b rounded-xl mb-4 ${
